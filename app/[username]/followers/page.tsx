...
export default function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            if (profile) {
                // Fetch follows where following_id is this user
                const { data: follows } = await supabase.from('follows').select('follower_id').eq('following_id', profile.id);
                if (follows && follows.length > 0) {
                    const followerIds = follows.map(f => f.follower_id);
                    const { data: profiles } = await supabase.from('profiles').select('*').in('id', followerIds);
        ...
        <AppShell>
            ...
            <div className="divide-y divide-neutral-800">
                {loading ? (
                    ...
                    <div className="p-12 text-center text-neutral-500">No followers yet.</div>
                ) : (
                    followers.map(user => (
